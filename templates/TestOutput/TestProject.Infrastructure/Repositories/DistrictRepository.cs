...
namespace TestProject.Infrastructure.Repositories
{
    public class DistrictRepository : RepositoryBase<District, int>, IDistrictRepository
    {
        ...
        public DistrictRepository(TestProjectContext context, IUnitOfWork unitOfWork, IStorageService storageService) 
        ...
        public async Task<DTResult<DistrictAggregate>> GetPagedAsync(DistrictDTParameters parameters)
        {
            ...
            var query = _context.Districts
                .Where(x => !x.IsDeleted)
                .Select(x => new DistrictAggregate
                {
                    Id = x.Id,
                    Name = x.Name,
            ...
            var data = new DTResult<DistrictAggregate>
            {
