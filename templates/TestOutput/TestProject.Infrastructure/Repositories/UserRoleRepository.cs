...
namespace TestProject.Infrastructure.Repositories
{
    public class UserRoleRepository : RepositoryBase<UserRole, int>, IUserRoleRepository
    {
        ...
        public UserRoleRepository(TestProjectContext context, IUnitOfWork unitOfWork, IStorageService storageService) 
        ...
        public async Task<DTResult<UserRoleAggregate>> GetPagedAsync(UserRoleDTParameters parameters)
        {
            ...
            var query = _context.UserRoles
                .Where(x => !x.IsDeleted)
                .Select(x => new UserRoleAggregate
                {
                    Id = x.Id,
                    Name = x.Name,
            ...
            var data = new DTResult<UserRoleAggregate>
            {
