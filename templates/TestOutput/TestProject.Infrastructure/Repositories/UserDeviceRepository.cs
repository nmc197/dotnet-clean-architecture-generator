...
namespace TestProject.Infrastructure.Repositories
{
    public class UserDeviceRepository : RepositoryBase<UserDevice, int>, IUserDeviceRepository
    {
        ...
        public UserDeviceRepository(TestProjectContext context, IUnitOfWork unitOfWork, IStorageService storageService) 
        ...
        public async Task<DTResult<UserDeviceAggregate>> GetPagedAsync(UserDeviceDTParameters parameters)
        {
            ...
            var query = _context.UserDevices
                .Where(x => !x.IsDeleted)
                .Select(x => new UserDeviceAggregate
                {
                    Id = x.Id,
                    Name = x.Name,
