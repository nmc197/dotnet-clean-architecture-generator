...
namespace TestProject.Application.Implements
{
    public class WardService : IWardService
    {
        ...
        {
            var isExistName = await _wardRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateWardDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _wardRepository.CreateListAsync(models);
            await _wardRepository.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetPagedAsync(SearchQuery query)
        {
            ...
            var pagedData = new PagingData<WardListDto>
            {
        ...
        public async Task<ApiResponse> GetPagedAsync(WardDTParameters parameters)
        {
        ...
        {
            var isExistName = await _wardRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateWardDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _wardRepository
            ...
            await _wardRepository.UpdateListAsync(existData);
            await _wardRepository.SaveChangesAsync();
