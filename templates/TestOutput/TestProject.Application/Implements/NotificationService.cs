...
namespace TestProject.Application.Implements
{
    public class NotificationService : INotificationService
    {
        ...
        {
            var isExistName = await _notificationRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateNotificationDto> objs)
        {
            ...
            await _notificationRepository.CreateListAsync(models);
            await _notificationRepository.SaveChangesAsync();
        ...
        {
            var isExistName = await _notificationRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _notificationRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateNotificationDto> obj)
        {
            ...
            await _notificationRepository.UpdateListAsync(existData);
            await _notificationRepository.SaveChangesAsync();
