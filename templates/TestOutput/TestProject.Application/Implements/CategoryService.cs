...
namespace TestProject.Application.Implements
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<ApiResponse> CreateAsync(CreateCategoryDto obj)
        {
            var isExistName = await _categoryRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateCategoryDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _categoryRepository.CreateListAsync(models);
            await _categoryRepository.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetPagedAsync(SearchQuery query)
        {
            ...
            var pagedData = new PagingData<CategoryListDto>
            {
        ...
        public async Task<ApiResponse> GetPagedAsync(CategoryDTParameters parameters)
        {
            var data = await _categoryRepository.GetPagedAsync(parameters);
            return ApiResponse.Success(data);
        }
        ...
        public async Task<ApiResponse> UpdateAsync(UpdateCategoryDto obj)
        {
            var isExistName = await _categoryRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _categoryRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();

            obj.ToEntity(existData);
            await _categoryRepository.UpdateAsync(existData);
            await _categoryRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateCategoryDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _categoryRepository
            ...
            await _categoryRepository.UpdateListAsync(existData);
            await _categoryRepository.SaveChangesAsync();
