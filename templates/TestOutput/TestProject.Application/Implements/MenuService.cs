...
namespace TestProject.Application.Implements
{
    public class MenuService : IMenuService
    {
        ...
        {
            var isExistName = await _menuRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateMenuDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _menuRepository.CreateListAsync(models);
            await _menuRepository.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetPagedAsync(SearchQuery query)
        {
            ...
            var pagedData = new PagingData<MenuListDto>
            {
        ...
        public async Task<ApiResponse> GetPagedAsync(MenuDTParameters parameters)
        {
            var data = await _menuRepository.GetPagedAsync(parameters);
            return ApiResponse.Success(data);
        }
        ...
        {
            var isExistName = await _menuRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            await _menuRepository.UpdateAsync(existData);
            await _menuRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateMenuDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _menuRepository
            ...
            await _menuRepository.UpdateListAsync(existData);
            await _menuRepository.SaveChangesAsync();
