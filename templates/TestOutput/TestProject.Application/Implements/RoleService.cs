...
namespace TestProject.Application.Implements
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateRoleDto> objs)
        {
        ...
        public async Task<ApiResponse> GetPagedAsync(RoleDTParameters parameters)
        {
            var data = await _roleRepository.GetPagedAsync(parameters);
            return ApiResponse.Success(data);
        }
        ...
        {
            var isExistName = await _roleRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateRoleDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _roleRepository
            ...
            await _roleRepository.UpdateListAsync(existData);
            await _roleRepository.SaveChangesAsync();
