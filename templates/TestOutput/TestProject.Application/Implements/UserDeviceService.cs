...
namespace TestProject.Application.Implements
{
    public class UserDeviceService : IUserDeviceService
    {
        ...
        public async Task<ApiResponse> CreateAsync(CreateUserDeviceDto obj)
        {
            var isExistName = await _userDeviceRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateUserDeviceDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _userDeviceRepository.CreateListAsync(models);
            await _userDeviceRepository.SaveChangesAsync();
        ...
        {
            var data = await _userDeviceRepository
                .FindByCondition(x => !x.IsDeleted)
                .Select(x => new UserDeviceListDto
                {
                    Id = x.Id,
                    Name = x.Name,
        ...
        {
            var data = _userDeviceRepository
                .FindByCondition(x => !x.IsDeleted)
                .Select(x => new UserDeviceListDto
                {
                    Id = x.Id,
            ...
            var pagedData = new PagingData<UserDeviceListDto>
            {
        ...
        public async Task<ApiResponse> GetPagedAsync(UserDeviceDTParameters parameters)
        {
        ...
        {
            var isDeleted = await _userDeviceRepository.SoftDeleteAsync(id);
            if (!isDeleted)
        ...
        public async Task<ApiResponse> UpdateAsync(UpdateUserDeviceDto obj)
        {
            var isExistName = await _userDeviceRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _userDeviceRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();

            obj.ToEntity(existData);
            await _userDeviceRepository.UpdateAsync(existData);
            await _userDeviceRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateUserDeviceDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _userDeviceRepository
            ...
            await _userDeviceRepository.UpdateListAsync(existData);
            await _userDeviceRepository.SaveChangesAsync();
