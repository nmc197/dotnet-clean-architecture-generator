...
namespace TestProject.Application.Implements
{
    public class UserStatusService : IUserStatusService
    {
        ...
        public async Task<ApiResponse> GetPagedAsync(UserStatusDTParameters parameters)
        {
        ...
        {
            var isExistName = await _userStatusRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _userStatusRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();

            obj.ToEntity(existData);
            await _userStatusRepository.UpdateAsync(existData);
            await _userStatusRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateUserStatusDto> obj)
        {
