...
namespace TestProject.Application.Implements
{
    public class SystemConfigService : ISystemConfigService
    {
        ...
        {
            var isExistName = await _systemConfigRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateSystemConfigDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _systemConfigRepository.CreateListAsync(models);
            await _systemConfigRepository.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetPagedAsync(SystemConfigDTParameters parameters)
        {
            var data = await _systemConfigRepository.GetPagedAsync(parameters);
            return ApiResponse.Success(data);
        }
        ...
        {
            var isExistName = await _systemConfigRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateSystemConfigDto> obj)
        {
            ...
            await _systemConfigRepository.UpdateListAsync(existData);
            await _systemConfigRepository.SaveChangesAsync();
