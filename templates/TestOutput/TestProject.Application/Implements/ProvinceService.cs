...
namespace TestProject.Application.Implements
{
    public class ProvinceService : IProvinceService
    {
        ...
        {
            var isExistName = await _provinceRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateProvinceDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _provinceRepository.CreateListAsync(models);
            await _provinceRepository.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetPagedAsync(SearchQuery query)
        {
            ...
            var pagedData = new PagingData<ProvinceListDto>
            {
        ...
        public async Task<ApiResponse> GetPagedAsync(ProvinceDTParameters parameters)
        {
        ...
        {
            var isExistName = await _provinceRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateProvinceDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _provinceRepository
            ...
            await _provinceRepository.UpdateListAsync(existData);
            await _provinceRepository.SaveChangesAsync();
