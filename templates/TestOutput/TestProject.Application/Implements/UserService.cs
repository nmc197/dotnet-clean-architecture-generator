...
namespace TestProject.Application.Implements
{
    public class UserService : IUserService
    {
        ...
        {
            var isExistName = await _userRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateUserDto> objs)
        {
            ...
            await _userRepository.CreateListAsync(models);
            await _userRepository.SaveChangesAsync();
        ...
        {
            var isExistName = await _userRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _userRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateUserDto> obj)
        {
            ...
            await _userRepository.UpdateListAsync(existData);
            await _userRepository.SaveChangesAsync();
