...
namespace TestProject.Application.Implements
{
    public class FileUploadService : IFileUploadService
    {
        ...
        public async Task<ApiResponse> CreateAsync(CreateFileUploadDto obj)
        {
            var isExistName = await _fileUploadRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateFileUploadDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _fileUploadRepository.CreateListAsync(models);
            await _fileUploadRepository.SaveChangesAsync();
        ...
        {
            var data = await _fileUploadRepository
                .FindByCondition(x => !x.IsDeleted)
                .Select(x => new FileUploadListDto
                {
                    Id = x.Id,
                    Name = x.Name,
        ...
        public async Task<ApiResponse> GetPagedAsync(SearchQuery query)
        {
            ...
            var pagedData = new PagingData<FileUploadListDto>
            {
        ...
        public async Task<ApiResponse> GetPagedAsync(FileUploadDTParameters parameters)
        {
            var data = await _fileUploadRepository.GetPagedAsync(parameters);
            return ApiResponse.Success(data);
        }

        public async Task<ApiResponse> SoftDeleteAsync(int id)
        {
            var isDeleted = await _fileUploadRepository.SoftDeleteAsync(id);
            if (!isDeleted)
        ...
        public async Task<ApiResponse> UpdateAsync(UpdateFileUploadDto obj)
        {
            var isExistName = await _fileUploadRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _fileUploadRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();

            obj.ToEntity(existData);
            await _fileUploadRepository.UpdateAsync(existData);
            await _fileUploadRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateFileUploadDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _fileUploadRepository
            ...
            await _fileUploadRepository.UpdateListAsync(existData);
            await _fileUploadRepository.SaveChangesAsync();
