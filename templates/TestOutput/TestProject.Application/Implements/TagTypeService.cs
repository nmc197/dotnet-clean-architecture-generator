...
namespace TestProject.Application.Implements
{
    public class TagTypeService : ITagTypeService
    {
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateTagTypeDto> objs)
        {
            ...
            await _tagTypeRepository.CreateListAsync(models);
            await _tagTypeRepository.SaveChangesAsync();
        ...
        public async Task<ApiResponse> GetPagedAsync(TagTypeDTParameters parameters)
        {
        ...
        {
            var isExistName = await _tagTypeRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
        ...
        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateTagTypeDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _tagTypeRepository
            ...
            await _tagTypeRepository.UpdateListAsync(existData);
            await _tagTypeRepository.SaveChangesAsync();
