...
namespace TestProject.Application.Implements
{
    public class AuditLogService : IAuditLogService
    {
        ...
        public async Task<ApiResponse> CreateAsync(CreateAuditLogDto obj)
        {
            var isExistName = await _auditLogRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
            ...
            await _auditLogRepository.CreateAsync(model);
            await _auditLogRepository.SaveChangesAsync();

            return ApiResponse.Created(model.Id);
        }

        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateAuditLogDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _auditLogRepository.CreateListAsync(models);
            await _auditLogRepository.SaveChangesAsync();
        ...
        {
            var data = await _auditLogRepository
        ...
        public async Task<ApiResponse> GetPagedAsync(SearchQuery query)
        {
            ...
            var pagedData = new PagingData<AuditLogListDto>
            {
        ...
        {
            var isDeleted = await _auditLogRepository.SoftDeleteAsync(id);
            if (!isDeleted)
        ...
        public async Task<ApiResponse> UpdateAsync(UpdateAuditLogDto obj)
        {
            var isExistName = await _auditLogRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _auditLogRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();

            obj.ToEntity(existData);
            await _auditLogRepository.UpdateAsync(existData);
            await _auditLogRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateAuditLogDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _auditLogRepository
            ...
            await _auditLogRepository.UpdateListAsync(existData);
            await _auditLogRepository.SaveChangesAsync();
