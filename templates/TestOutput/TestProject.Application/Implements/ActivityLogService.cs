...
namespace TestProject.Application.Implements
{
    public class ActivityLogService : IActivityLogService
    {
        ...
        {
            var isExistName = await _activityLogRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
        ...
        public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateActivityLogDto> objs)
        {
            var models = objs.Select(x => x.ToEntity());
            await _activityLogRepository.CreateListAsync(models);
            await _activityLogRepository.SaveChangesAsync();
        ...
        {
            var data = await _activityLogRepository
        ...
        public async Task<ApiResponse> GetByIdAsync(int id)
        {
            var data = await _activityLogRepository
                ...
                .Select(x => new ActivityLogDetailDto
                {
                    Id = x.Id,
                    Name = x.Name,
                    Description = x.Description,
        ...
        {
            var data = _activityLogRepository
                .FindByCondition(x => !x.IsDeleted)
                .Select(x => new ActivityLogListDto
                {
                    Id = x.Id,
            ...
            var pagedData = new PagingData<ActivityLogListDto>
            {
        ...
        {
            var isDeleted = await _activityLogRepository.SoftDeleteAsync(id);
            if (!isDeleted)
        ...
        public async Task<ApiResponse> UpdateAsync(UpdateActivityLogDto obj)
        {
            var isExistName = await _activityLogRepository.AnyAsync(x => !x.IsDeleted && x.Id != obj.Id && x.Name.ToLower() == obj.Name.ToLower());
            if (isExistName)
                return ApiResponse.UnprocessableEntity(
                    ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                    ApiCodeConstants.Common.DuplicatedData
            ...
            var existData = await _activityLogRepository.GetByIdAsync(obj.Id);
            if (existData == null)
                return ApiResponse.NotFound();

            obj.ToEntity(existData);
            await _activityLogRepository.UpdateAsync(existData);
            await _activityLogRepository.SaveChangesAsync();

            return ApiResponse.Success();
        }

        public async Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateActivityLogDto> obj)
        {
            var listIds = obj.Select(x => x.Id).ToList();
            var existData = await _activityLogRepository
            ...
            await _activityLogRepository.UpdateListAsync(existData);
            await _activityLogRepository.SaveChangesAsync();
