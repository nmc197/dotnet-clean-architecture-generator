...
(function () {
    ...
    const NotificationTagPage = {
        ...
        message: {
            pageTitle: I18n.t("notification_tag", "PAGE_TITLE"),
            ...
            createConfirm: I18n.t("common", "CREATE_CONFIRM", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            createSuccess: I18n.t("common", "CREATE_SUCCESS", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            createError: I18n.t("common", "CREATE_ERROR", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            updateConfirm: I18n.t("common", "UPDATE_CONFIRM", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            updateSuccess: I18n.t("common", "UPDATE_SUCCESS", { name: I18n.t("notification_tag", "PAGE_TITLE") }),
            updateError: I18n.t("common", "UPDATE_ERROR", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            deleteConfirm: I18n.t("common", "DELETE_CONFIRM", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            deleteSuccess: I18n.t("common", "DELETE_SUCCESS", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            deleteError: I18n.t("common", "DELETE_ERROR", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
            notFound: I18n.t("common", "NOT_FOUND", { name: I18n.t("notification_tag", "PAGE_TITLE").toLocaleLowerCase() }),
                ...
                ajax: {
                    ...
                    error: async function (xhr, status, error) {
                        if (xhr.status === 401) {
                            ...
                            const tableSettings = NotificationTagPage.table.settings()[0];
                            tableSettings.ajax.headers.Authorization = "Bearer " + TokenService.getAccessToken();

                            //Reload lại datatable với token mới
                            NotificationTagPage.table.ajax.reload();
                        }
                    },
                    data: function (d) {
                        d.isIncludeTagType = false;
                        d.tagTypeIds = [AppSettings.tagTypes.NOTIFICATION];
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row, meta) {
                            const info = NotificationTagPage.table.page.info();
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            return `<a href="#" class="btn btn-light btn-active-light-primary btn-flex btn-center btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-flip="top-end">
                                                ${NotificationTagPage.message.actions}
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                                        <div class="menu-item px-3">
                                            <a href="#" class="menu-link px-3 btn-edit" data-kt-docs-table-filter="edit_row" data-tag-id="${data}">
                                                ${NotificationTagPage.permissionFlags.canUpdate ? NotificationTagPage.message.edit : NotificationTagPage.message.detail}
                                            </a>
                                        </div>
                                        <div class="menu-item px-3 ${!NotificationTagPage.permissionFlags.canDelete ? "d-none" : ""}">
                                            <a href="#" class="menu-link px-3 btn-delete text-danger" data-kt-users-table-filter="delete_row" data-tag-id="${data}">
                                                ${NotificationTagPage.message.delete}
                                            </a>
                                        </div>
                                    </div>`
                        }
        ...
        checkPermissions: function () {
            if (!NotificationTagPage.permissionFlags.canCreate)
        ...
        bindClearFilterDateRangeEvent: function () {
            $("#clear_filter_created_date i").on("click", function () {
                NotificationTagPage.plugins.dateRangePickerFilter.clear();
            })
    ...
    /**
     * Handle add new notification tag
     */
    function addItem() {
        NotificationTagPage.formValidator.clearErrors();
        $("#kt_modal_tag_header h2").text(`${NotificationTagPage.message.create} ${NotificationTagPage.message.pageTitle.toLocaleLowerCase()}`);
        ...
        $("#btn_cancel_tag").text(NotificationTagPage.message.cancel);
        $("#kt_modal_tag_form input[type='text'],#kt_modal_tag_form textarea").attr("disabled", false);
     ...
     * Author: 
     * CreatedDate: 
     * Description: Edit notification tag by id
     * @param {number} id
     */
        ...
        try {
            ...
            Object.keys(data).forEach(key => {
                ...
                if (!key.toLocaleLowerCase().includes("date")) {
                    $(selector).attr("disabled", !NotificationTagPage.permissionFlags.canUpdate);
                }
            })
            if (NotificationTagPage.permissionFlags.canUpdate) {
                $("#kt_modal_tag_header h2").text(`${NotificationTagPage.message.edit} ${NotificationTagPage.message.pageTitle.toLocaleLowerCase()}`);
                $("#btn_save_tag").removeClass('d-none');
                $("#btn_cancel_tag").text(NotificationTagPage.permissionFlags.cancel);

            } else {
                $("#kt_modal_tag_header h2").text(`${NotificationTagPage.message.detail} ${NotificationTagPage.message.pageTitle.toLocaleLowerCase()}`);
                $("#btn_save_tag").addClass('d-none');
                $("#btn_cancel_tag").text(NotificationTagPage.permissionFlags.ok);
            }
            ...
            Swal.fire({
                icon: "error",
                title: NotificationTagPage.message.errorTitle,
                html: NotificationTagPage.message.notFound,
     ...
     * Author:
     * CreatedDate:
     * Description: Delete notification tag by id
     * @param {number} id
     */
    async function deleteItem(id) {
        ...
        const { isConfirmed } = await Swal.fire({
            icon: "question",
            title: NotificationTagPage.message.confirmTittle,
            html: NotificationTagPage.message.deleteConfirm,
        ...
        try {
            ...
            if (response?.isSucceeded) {
                NotificationTagPage.refreshDataTable();
                Swal.fire({
                    icon: "success",
                    title: NotificationTagPage.message.successTitle,
                    html: NotificationTagPage.message.deleteSuccess,
                    ...AppSettings.sweetAlertOptions(false)
                })
            }
        } catch (e) {
            ...
            Swal.fire({
                icon: "error",
                title: NotificationTagPage.message.failTitle,
                html: NotificationTagPage.message.deleteError,
                ...AppSettings.sweetAlertOptions(false)
            })
        }
        finally {
    ...
    /**
     * Save data (Create or Update) notification tag
     */
    async function saveData() {
        ...
        data["tagTypeId"] = AppSettings.tagTypes.NOTIFICATION;
        const isAdd = !data.id;
        const confirmText = isAdd ? NotificationTagPage.message.createConfirm : NotificationTagPage.message.updateConfirm;
        const { isConfirmed } = await Swal.fire({
            icon: 'question',
            title: NotificationTagPage.message.confirmTittle,
            html: confirmText,
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                if (response?.isSucceeded) {
                    ...
                    const successText = isAdd ? NotificationTagPage.message.createSuccess : NotificationTagPage.message.updateSuccess;
                    Swal.fire({
                        icon: "success",
                        title: NotificationTagPage.message.successTitle,
                        html: successText,
                ...
                AppUtils.handleApiError(e, {
                    ...
                    name: NotificationTagPage.message.pageTitle,
                    isShowAlert: true
                })
    ...
    function tableSearch() {
        NotificationTagPage.table.column(1).search($("#filter_name").val().trim());
        NotificationTagPage.table.column(2).search($("#filter_description").val().trim());
        NotificationTagPage.table.column(3).search($("#filter_created_date").val());
        NotificationTagPage.table.search($("#tag_datatable_search").val().trim()).draw();
    }
